...
internal class TacticalState : StateBase, IActionState
{
    ...
    private void FindCover()
    {
        ...
        for (float angle = 0; angle < 360; angle += Settings.CoverScanAngle)
        {
            ...
            var h = Bot.GetComponent<CharacterController>().Height / 2;
            ...
            if (trace.Hit && trace.GameObject != null)
            {
                ...
                if (pos is Vector3 vect && Bot.Scene.NavMesh.GetSimplePath(Bot.WorldPosition, vect).Any())
                {
                    if (Settings.ShowDebugOverlay)
                    {
                    ...
                    var path = Scene.NavMesh.GetSimplePath(Bot.WorldPosition, vect);
                    if (path.Any())
                    {
                        Bot.MoveTo(path);
                        return;
